...
def _point_to_proba(oof, y_train, preds, clip_val=30, padd=0.03):
    dat = list(zip(np.clip(oof, -clip_val, clip_val), np.where(y_train > 0, 1, 0)))
    dat = sorted(dat, key = lambda x: x[0])
    datdict = {dat[k][0]: dat[k][1] for k in range(len(dat))}

    spline_model = UnivariateSpline(list(datdict.keys()), list(datdict.values()))  
    spline_oof = spline_model(np.clip(oof, -clip_val, clip_val))
    spline_test = spline_model(np.clip(preds, -clip_val, clip_val))
    
    return np.clip(spline_oof, 0+padd, 1-padd), np.clip(spline_test, 0+padd, 1-padd)
...
def report_points(train, test, y_train, y_test, oof, preds, plot=True, **kwargs):
    ...
    brier_oof = round(brier_score_loss(y_true=np.where(y_train > 0, 1, 0), y_prob=spline_oof), 4)
    brier_test = round(brier_score_loss(y_true=np.where(y_test > 0, 1, 0), y_prob=spline_test), 4)
    ...
    print(f"Brier train: \t\t\t {brier_oof}")
    print(f"Brier test: \t\t\t {brier_test}")
    ...
    return pd.DataFrame({"mae_oof": [mae_oof], "mae_test": [mae_test], 
                         ...
                         "logloss_oof": [logloss_oof], "logloss_test": [logloss_test],
                          "brier_oof": [brier_oof], "brier_test": [brier_test],
...
def report_victory(y_train, y_test, oof, preds, probs=True, plot=True):
    ...
    if probs:
        ...
        brier_oof = round(brier_score_loss(y_true=y_train, y_prob=oof), 4)
        brier_test = round(brier_score_loss(y_true=y_test, y_prob=preds), 4)
...
def yearly_wrapper(train, test, y_train, y_test, oof, preds, min_yr=2022, points=True):
    y_train_total = []
    y_test_total = []
    oof_total = []
    preds_total = []
